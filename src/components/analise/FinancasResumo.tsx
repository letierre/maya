...
export function FinancasResumo() {
  ...
  const trend = [...byMonth.keys()]
    .sort()
    .slice(-6)
    .map((k) => {
      const { rec, des } = byMonth.get(k)!;
      return { k, saldo: rec - des };
    });
      ...
      <div style={{ ...CARD }}>
        ...
        {savingsPct != null && (
          <p style={{ margin: "10px 0 0", fontSize: 12, color: FOREGROUND, lineHeight: 1.4 }}>
            💰 Você poupou <span style={{ fontWeight: 700, color: savingsPct >= 0 ? GREEN : RED }}>{savingsPct}%</span> da sua renda este mês.
          </p>
        )}
        ...
        {trend.length >= 2 && (
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid oklch(0.28 0.02 270 / 0.5)" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 56 }}>
              {trend.map((t) => (
                <div key={t.k} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", gap: 4, height: "100%" }}>
                  <div style={{
                    width: "100%",
                  ...
                  <span style={{ fontSize: 9, color: MUTED, fontWeight: 600 }}>{monthShort(t.k)}</span>
                </div>
              ))}
            </div>
          </div>

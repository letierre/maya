...
export function LeituraResumo({ from, to }: { from: string; to: string }) {
  ...
  if (sessions.length > 0) {
    ...
    stats.push({ value: totalPages, label: t("an_paginas") });
    if (streak > 0) stats.push({ value: `${streak}d`, label: t("an_sequencia") });
  }
  if (doneBooks.length > 0) {
    stats.push({ value: doneBooks.length, label: t("an_livros_lidos") });
  }

  return (
    <Section title={t("leitura")}>
      <div style={{ ...CARD }}>
